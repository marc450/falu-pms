...
export default function AcceptInvitePage() {
  ...
  // The invite email links here with a token in the URL fragment. The JS
  // client parses it automatically (detectSessionInUrl) and fires an auth
  // state change — "SIGNED_IN" for invite links, "PASSWORD_RECOVERY" when
  // an admin re-sends access via a recovery email. Either one means we can
  ...
  useEffect(() => {
    ...
    // If no session materializes within 2.5 s, the link was probably
    // expired or already used. Show an explanatory state instead of
    // leaving the form gated forever.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Drop the invite session so the first real sign-in uses the
        ...
        <div className="bg-gray-800/60 border border-gray-700/80 rounded-2xl p-6 shadow-xl">
          ...
          {phase === "invalid" && (
            <div className="flex flex-col items-center text-center gap-3 py-2">
              ...
              <h2 className="text-white font-semibold text-base">Invite link expired or invalid</h2>
              <p className="text-gray-400 text-sm leading-relaxed">
                Invite links expire after 24 hours and can only be used once. Ask your administrator to send a new invite, or request a password reset if your account already exists.
              </p>
              <Link
          ...
          {phase === "done" && (
            <div className="flex flex-col items-center text-center gap-3 py-2">
              ...
              <h2 className="text-white font-semibold text-base">You&apos;re all set</h2>
              <p className="text-gray-400 text-sm leading-relaxed">
