-- ============================================================
-- Migration 094a: correct shift-boundary fix (true window, MAX/MIN)
-- ============================================================
-- 093a's per-reading reset detection was too fragile: real PLC
-- counters have small backward jitter mid-shift (cur drops a
-- few ticks below prev), and the per-reading rule read every
-- such blip as a full reset and added the entire cur value.
-- Result: ~33000s of "production" in a 5-min bucket. Reverted.
--
-- Correct fix = 089e's per-bucket math (jitter-immune, because
-- MAX/MIN absorb blips) with the ONE real defect removed: the
-- ±1min padded read window. That padding was the sole cause of
-- the boundary gap — it pulled the pre-reset high-water mark
-- into the first post-reset bucket so MAX ~= anchor and every
-- delta collapsed to 0. With the true [bucket_start, bucket_end)
-- window the boundary bucket is purely post-reset, hits the
-- reset branch (MAX < anchor), and delta = MAX − MIN > 0.
-- Removing the +1min tail also removes the over-attribution one
-- bucket later.
--
-- _end_* stays = MAX (matches 089f / 094b anchor chain).
...
BEGIN
  ...
  LOOP
    ...
    -- delta = MAX − MIN is the growth that happened INSIDE this bucket.
    -- Normal branch (MAX >= anchor): delta = MAX − anchor.
    v_delta_swabs   := CASE
