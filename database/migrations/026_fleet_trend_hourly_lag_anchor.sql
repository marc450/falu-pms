-- Migration 026: get_fleet_trend — fix LAG anchor for hourly path
--
-- Migration 025 introduced LAG()-based incremental BU calculation for the
-- hourly path.  However, the WHERE clause still filtered shift_readings to
-- recorded_at >= range_start.  This means the very first reading of any
-- shift that falls inside the window has no previous row in the partition,
-- LAG() returns NULL, and COALESCE(NULL, 0) causes that first bar to show
-- the FULL cumulative production since shift-start (e.g. 8 hours of swabs
-- in a single 1-hour bar) instead of just the incremental for that hour.
--
-- Fix: widen the hourly WHERE clause to range_start - 12 hours (maximum
-- shift length) so that the previous cumulative reading is always fetched
-- and available as the LAG anchor.  The final SELECT then filters out any
-- hour_bucket that falls before the originally requested range_start.
