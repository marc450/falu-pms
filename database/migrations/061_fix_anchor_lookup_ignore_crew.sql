-- ============================================================
-- Migration 061: Fix anchor lookup to ignore shift_crew
-- ============================================================
-- PLC cumulative counters do NOT reset when crews change.
-- The anchor lookup was filtering by shift_crew, so at every
-- crew transition it found no prior row, defaulted to 0, and
-- attributed the entire cumulative counter as one hour's delta.
--
-- Fix: look up the most recent hourly_analytics row for the
-- same machine regardless of which crew was working.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- ── Anchor lookup: find the most recent hourly_analytics row
    -- ── for this machine, REGARDLESS of shift_crew.
    -- ── PLC cumulative counters span across crew changes.
    SELECT
      _end_produced_swabs,
      _end_produced_boxes,
