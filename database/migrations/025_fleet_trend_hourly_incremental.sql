-- Migration 025: get_fleet_trend — separate hourly and daily aggregation paths
--
-- Migration 024 applied the 7-hour work-day offset unconditionally (both granularities).
-- For HOURLY granularity this caused two problems:
--
--   1. The range was widened by 7 hours → extra bars appeared before the
--      requested start time.
--
--   2. The MAX-per-(machine,shift,hour) trick that works for daily granularity
--      is wrong for hourly: the last cumulative reading of a 12-hour shift
--      (recorded_at = 19:00, produced_swabs = full-shift total) landed in the
--      18:00 hourly bucket, making that bar look like it produced the entire
--      shift worth of swabs in one hour.
--
-- Fix — two separate paths:
--
--   HOURLY  — LAG()-based incremental production: subtract the previous
--              cumulative reading from the current one to get the swabs
--              produced in that specific 1-hour window. No 7-hour shift.
--
--   DAILY   — unchanged MAX + 7-hour work-day offset from migration 024.
--
-- analytics_readings is already incremental, so SUM works for both
-- granularities; only the daily path applies the 7-hour bucket offset.
...
-- GREATEST(0,…) guards against rare out-of-order or reset artefacts.
sr_hour_inc AS (
  SELECT
...
-- Step H3: park-level aggregation ─────────────────────────────────────────────
sr_hour_agg AS (
  SELECT
    bucket,
    ...
    1::bigint                           AS shift_count  -- not used in hourly KPI
  FROM sr_hour_inc
  GROUP BY bucket
),
...
sr_day_src AS (
  ...
  WHERE bucket_granularity = 'day'
    -- Widen by 7 h to capture night-shift readings (00:00–06:59) that belong
    -- to the preceding work-day.
...
-- ══════════════════════════════════════════════════════════════════════════════
-- Combined sr result (union of the two paths)
-- ══════════════════════════════════════════════════════════════════════════════
...
-- For daily: exclude work-day buckets before the actual range start
-- (they can appear because sr_day_src widens the range by 7 hours).
WHERE bucket_granularity = 'hour'
