-- ============================================================
-- Migration 089f: rerun 48h backfill with MIN-based reset
-- ============================================================
-- Part 2 of the 089e fix — TRUNCATEs bucket_analytics_5m and
-- re-derives every bucket in the 48h window using the same
-- MIN-based delta math the new aggregator (089e) writes, so
-- the chart heals immediately instead of waiting 48h for the
-- spikes to age out of retention.
--
-- Set-based pass mirrors 089c; the only change is per-counter
-- MIN aggregates surfaced into the agg CTE and used in the
-- reset branch of each delta CASE.
--
-- Requires 089e. Safe to re-run (TRUNCATE + INSERT is
-- transactional; readers see either the old state or the
-- fully repopulated table, never empty).
-- ============================================================
...
SELECT
  ...
  bucket_ts,
  shift_crew,
  -- Reset branch now subtracts MIN (first post-reset reading in this bucket)
  -- instead of using MAX whole — that's what tames the post-shift spike.
  CASE
