-- ============================================================
-- Migration 042: machine_code as primary lookup in hourly_analytics
-- ============================================================
-- machine_code (the PLC numeric UID e.g. "11550") never changes.
-- machine_id (UUID) is a Supabase-internal row key that changes if a
-- machine is ever deleted and re-registered. Using machine_id as the
-- UNIQUE constraint and anchor lookup key therefore risks silent delta
-- spikes after a re-registration. This migration switches both to
-- machine_code. machine_id remains in the table as a safety reference.
-- ============================================================

-- ── 1. Swap UNIQUE constraint on hourly_analytics ────────────────────────────

ALTER TABLE hourly_analytics
  DROP CONSTRAINT IF EXISTS hourly_analytics_machine_id_plc_hour_shift_number_key;

ALTER TABLE hourly_analytics
  ADD CONSTRAINT hourly_analytics_machine_code_hour_shift_key
  UNIQUE (machine_code, plc_hour, shift_number);

-- Replace UUID-based index with machine_code-based index
DROP INDEX IF EXISTS hourly_analytics_machine_plc_idx;

CREATE INDEX IF NOT EXISTS hourly_analytics_machine_code_plc_idx
  ON hourly_analytics (machine_code, plc_hour DESC);


-- ── 2. Recreate aggregate_cell_hour with machine_code lookups ────────────────
-- Three changes from migration 041:
--   A) Inner aggregation query: WHERE machine_code = v_machine_code (not machine_id)
--   B) Anchor lookup:           WHERE machine_code = v_machine_code (not machine_id)
--   C) ON CONFLICT target:      (machine_code, plc_hour, shift_number) (not machine_id)
...
BEGIN
  ...
  LOOP
    -- A. Aggregate by machine_code (not machine_id)
    SELECT
      COUNT(*)                         AS rdg_count,
    ...
    FROM shift_readings sr
    WHERE sr.machine_code = v_machine_code        -- changed from machine_id
    ...
    -- B. Anchor lookup by machine_code (not machine_id)
    SELECT
      _end_produced_swabs,
      _end_produced_boxes,
    ...
    WHERE machine_code = v_machine_code           -- changed from machine_id
    ...
    -- C. ON CONFLICT uses machine_code (not machine_id)
    ...
    ON CONFLICT (machine_code, plc_hour, shift_number) DO UPDATE SET  -- changed from machine_id
